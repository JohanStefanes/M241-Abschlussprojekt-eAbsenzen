import type { Route } from "./+types/lernende";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const student = user.studentName ?? user.displayName;
...
export default function LernendeDashboard({ loaderData }: Route.ComponentProps) {
  const { absences, stats, student, filter } = loaderData;

  return (
    <div className="space-y-6">
      <PageHeader title="Meine Absenzen" subtitle={`Angemeldet als ${student}`} />
      <StatsSummary stats={stats} />
      <FilterBar active={filter} />
      <AbsenceTable absences={absences} basePath="/lernende/absenz" />
    </div>
  );
}
